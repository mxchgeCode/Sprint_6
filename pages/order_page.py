...
class YaScooterOrderPage(BasePage):
    ...
    @allure.step('Переходим на следующий этап заказа')
    def go_next(self):
    ...
    @allure.step('Выбираем периода аренды')
    ...
    @allure.step('Переходим к статусу заказа')
    def click_go_to_status(self):
        return self.find_element(Locators.SHOW_STATUS_BUTTON).click()

    @allure.step('Заполняем данные на этапе "Для кого самокат"')
    ...
    @allure.step('Заполняем данные на этапе "Про аренду"')
